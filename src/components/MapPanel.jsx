  ...
  return (
    <div className="relative h-80 overflow-hidden rounded-3xl border border-white/40 bg-gradient-to-br from-white via-white to-brand-sand">
      ...
      <div className="relative p-6">
        <div className="text-xs font-semibold uppercase tracking-[0.3em] text-brand-navy">{t("home.mapTitle")}</div>
        <div className="mt-2 text-sm text-brand-slate">{t("map.pinnedCopy")}</div>
      </div>
      {pins.map((pin, index) => (
        <div
          key={pin.id}
      ...
      <div className="absolute bottom-4 right-4 rounded-full bg-white/80 px-4 py-2 text-xs text-brand-slate">
        {t("search.map")}
      </div>
    </div>
  )
